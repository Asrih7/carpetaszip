...
export class ModuleDetailPage implements AfterViewInit {
  ...
  async initializeDocs() {
    ...
    this.storage.get('usuario').then((val) => {
      if (val != null && val != undefined){
        ...
        this.proveedor.consultarDocsCursoMesTema(this.usuario.Mensaje.AuthorizationToken, this.idCurso, this.mesAcceso, this.modulo.IdBloque, this.modulo.IdTema)
        .subscribe(
  ...
  initializeVideos() {
    ...
    this.storage.get('usuario').then((val) => {
      if (val != null && val != undefined){
        ...
        this.proveedor.consultarVideosCursoMesTema(this.usuario.Mensaje.AuthorizationToken, this.idCurso, this.mesAcceso, this.modulo.IdBloque, this.modulo.IdTema)
        .subscribe(
  ...
  initializeTest() {
    ...
    this.storage.get('usuario').then((val) => {
      if (val != null && val != undefined){
        ...
        this.proveedor.consultarTestCursoMesTema(this.usuario.Mensaje.AuthorizationToken, this.idCurso, this.mesAcceso, this.modulo.IdBloque, this.modulo.IdTema)
        .subscribe(
  ...
  hacerTest(item) {
    ...
    if (item.Pendiente > 0)
    {
        ...
        this.navParamService.setNavData(JSON.stringify({idOposicion: this.idOposicion, test: item, numMinutos: this.numMinutos, numPreguntas: this.numPreguntas, continuar: this.continuar, origen: this.modulo, etiqueta: null, revisar: false}));
  ...
  parseTestJson(data, nombre, mediaAlumnos)
  {
    if (data.Error == 0){
      ...
      this.navParamService.setNavData(JSON.stringify({idOposicion: this.idOposicion, test: this.test, tipoTest: this.tipoTest, valorFallo: data.Mensaje.ValorFallo, listaPreguntas: this.listaPreguntas, listaRespuestas: this.listaRespuestas, aciertos: this.aciertos, fallos: this.fallos, blancos: this.blancos, dudosas: this.dudosas, dudosasAcierto: this.dudosasAcierto, dudosasFallo: this.dudosasFallo, nota: this.decimalPipe.transform(this.nota/10, '1.2-2'), notaRiesgo: this.decimalPipe.transform(this.notaRiesgo/10, '1.2-2'), mediaAlumnos: mediaAlumnos, origen: this.modulo, etiqueta: null, revisar: true}));
